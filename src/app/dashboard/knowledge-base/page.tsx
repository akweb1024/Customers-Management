...
export default function KnowledgeBasePage() {
    ...
    const categories = ['MY WORK', 'ALL', 'FAQ', 'SOP', 'Product Guide', 'Internal Policy'];

    useEffect(() => {
        const userData = localStorage.getItem('user');
        if (userData) setUserRole(JSON.parse(userData).role);
        // We don't fetch articles initially if defaulting to MY WORK
    }, []);

    const fetchArticles = async (cat?: string) => {
        if (cat === 'MY WORK') return;
        setLoading(true);
        try {
