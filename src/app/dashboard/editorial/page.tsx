...
export default function EditorialPage() {
    ...
    const fetchJournals = async () => {
        try {
            // Reusing existing Journals API or assuming a similar endpoint exists
            const token = localStorage.getItem('token');
            // Assuming /api/journals exists (it was in the original schema context, user has journals)
            // If not, we might fail here. Let's assume there's a way to get journals.
            // If /api/journals is protected or different, we might need a specific simplified endpoint.
            // For now, let's try /api/journals but if it fails handle gracefully.
            const res = await fetch('/api/journals', { headers: { 'Authorization': `Bearer ${token}` } });
            if (res.ok) {
                const data = await res.json();
                // API might return { journals: [...] } or just [...]
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="card p-4 bg-white border border-secondary-200">
                    <h4 className="text-xs font-bold text-secondary-400 uppercase">Pending Review</h4>
                    <p className="text-2xl font-black text-secondary-900">{articles.filter(a => a.status === 'SUBMITTED').length}</p>
                </div>
                <div className="card p-4 bg-white border border-secondary-200">
                    <h4 className="text-xs font-bold text-secondary-400 uppercase">Under Review</h4>
                    <p className="text-2xl font-black text-warning-600">{articles.filter(a => a.status === 'UNDER_REVIEW').length}</p>
                </div>
                <div className="card p-4 bg-white border border-secondary-200">
                    <h4 className="text-xs font-bold text-secondary-400 uppercase">Revisions</h4>
                    <p className="text-2xl font-black text-primary-600">{articles.filter(a => a.status === 'REVISION_REQUESTED').length}</p>
                </div>
                <div className="card p-4 bg-white border border-secondary-200">
                    <h4 className="text-xs font-bold text-secondary-400 uppercase">Accepted</h4>
                    ...
                    <tbody className="divide-y divide-secondary-100">
                        {articles.map(article => (
                            <tr key={article.id} className="hover:bg-secondary-50">
                                ...
                                <td className="p-4">{article.authors?.[0]?.name}</td>
                        ...
                        {articles.length === 0 && (
                            <tr>
                                <td colSpan={6} className="p-8 text-center text-secondary-400">No articles found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {isSubmitting && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-lg w-full">
                        <h3 className="text-xl font-bold mb-4">Submit New Manuscript</h3>
