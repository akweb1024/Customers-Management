    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4">
                {events.map(event => (
                    <div key={event.id} className="card-premium p-6 flex flex-col md:flex-row gap-6 hover:shadow-lg transition-all group">
                        <div className="w-full md:w-48 bg-secondary-100 rounded-2xl flex flex-col items-center justify-center p-4 text-center">
                            ...
                            <span className="text-xs text-secondary-500">{new Date(event.startDate).getFullYear()}</span>
                        </div>
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                ...
                                <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase ${new Date(event.endDate) < new Date() ? 'bg-secondary-200 text-secondary-500' : 'bg-success-100 text-success-700'}`}>
                            ...
                            <div className="mt-4 pt-4 border-t border-secondary-100 flex gap-6 text-sm font-bold text-secondary-700">
                                <span>🎟️ {event._count?.registrations || 0} Registered</span>
                                <span>📄 {event._count?.papers || 0} Abstract Submissions</span>
                            </div>
                        </div>
            ...
            {events.length === 0 && !loading && (
                <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-secondary-200">
                    ...
                    <p className="text-secondary-500">Create an event to start accepting registrations.</p>
                </div>
            )}
        </div>
    );
